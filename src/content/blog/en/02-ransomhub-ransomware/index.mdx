...
<Figure
  label="fig.1"
  caption="Daemonisation: everything the locker does before it touches a single file."
  alt="Flow diagram of the ransomware's daemonisation. The locker starts and checks whether -verbose was given; without it, it daemonises. It calls fork(); the parent goes to waitpid() and reaps the child, while the child calls setsid(), exiting with status 1 if that fails. The child then closes file descriptors 0, 1 and 2, reopens /dev/null and duplicates it over them, and only then enters the encrypt loop."
>
{`
             ./locker
                 │
